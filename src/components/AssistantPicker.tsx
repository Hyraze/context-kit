...
export default function AssistantPicker({ selected, onChange }: Props) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-white">Choose your AI assistant(s)</h2>
        ...
        {ASSISTANTS.map(assistant => {
          ...
          return (
            ...
            >
              ...
              <div>
                <div className="font-semibold text-zinc-100">{assistant.name}</div>
                <div className="text-xs text-violet-400 mt-0.5">{assistant.badge}</div>
              </div>
              <p className="text-xs text-zinc-500 leading-relaxed">{assistant.description}</p>
            </button>
          )
        })}
      </div>
      ...
      {selected.length > 1 && (
        <p className="text-xs text-zinc-500 bg-zinc-900 border border-zinc-800 rounded-lg px-4 py-3" role="note">
          Shared files (like <code className="text-violet-400">.env.example</code>) are included once.
